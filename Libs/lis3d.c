...
HAL_StatusTypeDef lis3dh_init(lis3dh_t *lis3dh, I2C_HandleTypeDef *i2c, uint8_t *buf, uint16_t bufsize) {
    ...
    /* Initialize EMA smoothing defaults */
    lis3dh->ema_alpha_q15 = LIS3DH_EMA_ALPHA_Q15_DEFAULT;
    lis3dh->ema_initialized = 0;
    lis3dh->x_raw = lis3dh->y_raw = lis3dh->z_raw = 0;
    lis3dh->ema_x = lis3dh->ema_y = lis3dh->ema_z = 0;
    ...
    status = lis3dh_write(lis3dh, REG_CTRL_REG1, DATA_RATE_LOW_100Hz | 0x07);
    if (status != HAL_OK) return status;
...
HAL_StatusTypeDef lis3dh_get_xyz(lis3dh_t* lis3dh) {
    ...
    /* Decode raw 16-bit signed values from low,high bytes */
    int32_t raw_x = (int32_t)((((int8_t) lis3dh->buf[1]) << 8) | lis3dh->buf[0]);
    int32_t raw_y = (int32_t)((((int8_t) lis3dh->buf[3]) << 8) | lis3dh->buf[2]);
    int32_t raw_z = (int32_t)((((int8_t) lis3dh->buf[5]) << 8) | lis3dh->buf[4]);

    lis3dh->x_raw = raw_x;
    lis3dh->y_raw = raw_y;
    lis3dh->z_raw = raw_z;

    if (!lis3dh->ema_initialized) {
        lis3dh->ema_x = raw_x;
        lis3dh->ema_y = raw_y;
        lis3dh->ema_z = raw_z;
        lis3dh->ema_initialized = 1;
    } else {
        /* Fixed-point EMA: y += alpha * (x - y) >> 15, alpha in Q15 */
        const int32_t alpha = (int32_t)lis3dh->ema_alpha_q15;
        int32_t dx = (int32_t)(raw_x - lis3dh->ema_x);
        int32_t dy = (int32_t)(raw_y - lis3dh->ema_y);
        int32_t dz = (int32_t)(raw_z - lis3dh->ema_z);
        lis3dh->ema_x += (int32_t)((alpha * dx) >> 15);
        lis3dh->ema_y += (int32_t)((alpha * dy) >> 15);
        lis3dh->ema_z += (int32_t)((alpha * dz) >> 15);
    }

    /* Publish filtered values */
    lis3dh->x = (int)lis3dh->ema_x;
    lis3dh->y = (int)lis3dh->ema_y;
    lis3dh->z = (int)lis3dh->ema_z;
